...
<script language="javascript">
	...
	//添加
	function doCreate(){
		window.open("${ctx}/system/configManage/SMSConfig/create/index.jsp","新增短信配置" ,"top=100, left=100, width=900,height=600,toolbar=no");	
	}
	//编辑
	function doEdit(){
		if(!getId()){
		...
		if (count > 1) {
			alert("只能选择一个短信配置进行操作！");
			return;
		}
		window.open("${ctx}/system/configManage/SMSConfig/updatePrepare.do?geSmsConfig.smsId=" + idStr,"编辑短信配置" ,"top=100, left=100, width=900,height=600,toolbar=no");		
	}
	...
	function doDelete(){
		...
		if (count > 1) {
			alert("只能选择一个短信配置进行删除！");
			return;
		}
		if(confirm("确定删除您选中的短信配置吗？")){
			window.parent.frames[4].location.href = "${ctx}/system/configManage/SMSConfig/deleteSms.do?geSmsConfig.smsId=" + idStr;
		}
	}
	//查看
	function doView(){
		...
		window.open("${ctx}/system/configManage/SMSConfig/viewSms.do?geSmsConfig.smsId=" + idStr,"查看短信配置" ,"top=100, left=100, width=900,height=600,toolbar=no");
	}
	//特殊处理end
</script>
</HTML>
