<%@page import="cn.com.sinosoft.ebusiness.common.bizConfig.domain.GeCodeType"%>
...
<body topmargin="2" leftmargin="2">
	...
	<c:forEach items="${geCodeTypeList}" var="geCodeType" begin="0" end="${totalCount}" step="1" varStatus="status">
		<tr id="tr_${status.index}"  class="${status.index%2 == 0?'':'search_tr_ou'}">
			...
			-->
			<td class="search_body"  width="125px;" ><a href="javascript:doSearchSon('id_${status.index}')">${geCodeType.codeTypeCDesc}</a></td>
...
<script type="text/javascript">
	...
	function doSearchSon(obj){
		var idStr = document.getElementById(obj).value;
		window.open("${ctx}/business/businessManage/dataDictionary/queryGeCodePage.do?geCodeType.codeType="+idStr,"数据字典类型下的数据" ,"top=100, left=100, width=900,height=600,toolbar=no");		
	}
</script>
