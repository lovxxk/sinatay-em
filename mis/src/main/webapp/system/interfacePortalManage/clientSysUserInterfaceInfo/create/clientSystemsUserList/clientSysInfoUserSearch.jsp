...
<body topmargin="2" leftmargin="2">
	<table width="100%" class="table_Show" id="data_table">
		<tr>
			...
			<td class="search_head" nowrap>外部系统ID</td>
		...
		<c:forEach items="${page.result}" var="clientUser" begin="0" end="${page.totalCount}" step="1" varStatus="st">
			<tr id="tr_${st.index}" class="${st.index%2 == 0?'':'search_tr_ou'}">
				<td class="search_body_center" nowrap>
					<input type="radio"" name="checkChild" id="check${st.index}" onclick="checkSingleRow('${st.index}');" value="${clientUser.id}">
				</td>
				<td class="search_body_center" nowrap>${st.index+1+page.pageSize*(page.currentPageNo-1)}</td>
				<td class="search_body_center" nowrap>${clientUser.id}</td>
				<td class="search_body_center" nowrap>${clientUser.externalSysInfo.externalSysId}</td>
				<td class="search_body_center" nowrap>${clientUser.loginName}</td>
				<td class="search_body_center" nowrap>
					<c:if test="${clientUser.status=='0'}">启用</c:if>
					<c:if test="${clientUser.status=='1'}">停用</c:if>
				</td>
				<td class="search_body_center" nowrap>${clientUser.createDate}</td>
				<td class="search_body_center" nowrap>${clientUser.updateDate}</td>
				<td class="search_body_center" nowrap>${clientUser.remark}</td>
			</tr>
		</c:forEach>		
	</table>
</body>
