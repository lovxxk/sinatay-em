	...
	<div id="toolbar_DIV">
		<table id="toolbar_main" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td onclick="getProduct();" onmouseover="changeTD_style(this);" onmouseout="changeTD_style(this);">关联到集团做服务器端时为外部系统分配的用户</td>
				<td>&nbsp;</td>
			</tr>
		</table>
	</div>
...
<script language="javascript">
	...
	function getProduct(){
		...
		if (count == 1) {
			var area = valArr[0];
			if (confirm("确定关联该外部系统信息ID?")) {
				top.opener.document.getElementById("externalSysId").value = valArr[0];
			}
			top.close();
		} else if (count == 0) {
			alert("请选择要关联的外部系统用户！");
