	...
	<div id="toolbar_DIV">
		<table id="toolbar_main" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td onclick="getProduct();" onmouseover="changeTD_style(this);" onmouseout="changeTD_style(this);">关联到服务端用户和接口间关系</td>
				<td>&nbsp;</td>
			</tr>
		</table>
	</div>
...
<script language="javascript">
	...
	function getProduct(){
		...
		if (count == 1) {
			var area = valArr[0];
			if (confirm("确定关联该服务端用户主键?")) {
				top.opener.document.getElementById("id").value = valArr[0];
				top.opener.document.getElementById("id").style.color = "black";
			}
			top.close();
		} else if (count == 0) {
			alert("请选择要关联的服务端用户！");
