	...
	<div id="toolbar_DIV">
		<table id="toolbar_main" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td onclick="getProduct();" onmouseover="changeTD_style(this);" onmouseout="changeTD_style(this);">关联到外部系统用户和接口之间的关系</td>
				<td>&nbsp;</td>
			</tr>
		</table>
	</div>
...
<script language="javascript">
	...
	function getProduct(){
		...
		if (count == 1) {
			var area = valArr[0];
			if (confirm("确定关联该接口信息ID?")) {
				top.opener.document.getElementById("transCode").value = valArr[0];
				top.opener.document.getElementById("transCode").style.color = "black";
			}
			top.close();
		} else if (count == 0) {
			alert("请选择要关联的接口信息！");
