...
<body topmargin="2" leftmargin="2">
	<table width="100%" class="table_Show" id="data_table">
		<tr>
			...
			<td class="search_head" nowrap>接口状态</td>
		...
		<s:iterator value="page.result" var="interfaceInfo" status="st">
			<tr id="tr_${st.index}">
				...
				<td class="search_body_center" nowrap>					
  					<s:if test='#interfaceInfo.transType == "K"'>客户端</s:if>
					<s:if test='#interfaceInfo.transType == "F"'>服务端</s:if>
				</td>
				<td class="search_body_center" nowrap>
					<s:if test="#interfaceInfo.status == 1">启用</s:if>
					<s:if test="#interfaceInfo.status == 2">停用</s:if>
				</td>
