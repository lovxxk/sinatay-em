...
<script language="javascript">
	...
	function doDelete(){
		...
		if(confirm("确定删除您选中的服务网点吗？")){
			$.ajax({
				url:contextRootPath+"/serviceNetwork/deleteServiceNetwork.do",
				async:false,
				dataType:"json",
				data:{"riskCode": idStr 
				     },
	...
	//	alert("查看的代码："+idStr);
		window.open("${ctx}/serviceNetwork/viewServiceNetwork.do?network.manageCom=" + idStr,"查看服务网点" ,"top=100, left=100, width=900,height=600,toolbar=no");
	}
	function doUpate(){
		if(!getId()){
			return;
	...
	//	alert("更新的代码："+idStr);
		window.open("${ctx}/serviceNetwork/prepareUpdateNetwork.do?network.manageCom="+idStr,"查看服务网点" ,"top=100, left=100, width=900,height=600,toolbar=no");
	}
