...
<script language="javascript">
	...
	//特殊处理，随功能不同而不同
	//绑定运单号
	function boundWayBill(){
		if (!getId()){
			return;
		}
		if (count == 0) {
			alert("请选择要进行物流维护的订单记录！");
			return;
		}
		if (count > 1) {
			alert("只能选择一条订单记录进行操作！");
			return;
		}
		var orderArr = idStr.split("|");
		//if (orderArr[1] == "1" ){
		//	alert("您不能对车险订单进行物流维护操作！");
		//	return;
		//}else{
		window.open("${ctx}/business/businessManage/onlineOrderManage/queryGeThirdParterInfoByList.do?geOrder.orderNo=" + orderArr[0] +"&geOrder.flag=" + orderArr[1],"绑定运单号" ,"top=200, left=200, width=650,height=200,toolbar=no");
		//}
	}
</script>
</HTML>