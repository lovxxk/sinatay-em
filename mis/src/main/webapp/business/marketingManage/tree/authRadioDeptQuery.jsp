...
function doSearch(){
	//只能写在树里面
	var authorityDepartmentManager = window.top.opener.document.getElementById("authorityDepartmentManager");
	var authorityid = window.top.opener.document.getElementById("authorityid");
	
	if(authorityid!=departmentIds&&authorityid!=""){//
		//清空产品
		var coreProductNames = window.top.opener.document.getElementsByName("coreProductName");
		var coreProductCodes  = window.top.opener.document.getElementsByName("coreProductCode");
		for(var i=1;i<coreProductNames.length;i++){
			coreProductNames[i].value="";
			coreProductCodes[i].value="";
		}
		
		//若地区改，1.修改第三方产品
		//修改加购产品
		//改第三方产品
		var activityPatterns = window.top.opener.document.getElementsByName("activityPattern");
		var itemNames = window.top.opener.document.getElementsByName("itemName");
		var itemIDs =  window.top.opener.document.getElementsByName("itemID");
		for(var i=1;i<activityPatterns.length;i++){
			if(activityPatterns[i].value=="1"||activityPatterns[i].value=="2"||activityPatterns[i].value=="3"){//面向所有商品 //面向前N名客户 //抽奖
				itemNames[i].value="";
				itemIDs[i].value="";
			}
			if(activityPatterns[i].value=="5"){//加购
				//将所有的加购产品
				clearAddShopping(activityPatterns[i]);//清空加购产品
			}
		}
	}
...
function clearAddShopping(field){
