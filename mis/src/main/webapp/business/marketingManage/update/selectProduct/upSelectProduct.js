...
function recommendProductSrc(){
	...
	if($("#eids").val()!=null&&$("#eids").val()!=undefined){
		var eids=$("#eids").val();
	}
	...
	if (count > 0) {
		...
		for (var i = 0; i < noRecommendProductTrIdStrs.length; i++) {
			...
			if(!containt(eid,'recommendProduct')){//如果不包含则往右侧添加
				...
				appendHTML = appendHTML+"<td class='td_border_body_left_top_none' nowrap style='height: 30px'>"+eid+"</td>";
...
function saveRecommendProduct(){
	...
	var obj = document.getElementById("recommendProduct");
		...
		//alert(xRule+"--- yAddShopping-- "+yAddShopping);
		var TRObj = top.opener.document.getElementById(xRule);
		if($.getUrlParam('operation')!=null&&$.getUrlParam('operation')!=undefined&&$.getUrlParam('operation')==0){
			...
			for(var i=0;i<eids.length;i++){
				if(eids[i]!=""&&eids[i]!=undefined&&productNames[i]!=""&&productNames[i]!=undefined){
						...
						appendHtml = appendHtml+"<td>"+(i+1)+".</td>";
						appendHtml = appendHtml+"<td>"+"<input type='hidden' class='coreProductCode' name='coreProductCode' value='"+eids[i]+"'>"+"</td>";
						if(discount=="4"){
							...
							appendHtml = appendHtml+"<td align='right' class='clazz'><span style=\"font-weight: bold;padding-left: 200px\"> 折扣ID(选填)：</span><input class=\"activityPatternDiscountID\" id=\"activityDiscountID"+i+"\" name=\"geAddServiceActivity.geActivitiesProducts["+i+"].discountID\" style=\"width: 170px;\" onkeyup=\"value=value.replace(/[\\\W]/g,'')\" type=\"text\" maxLength=\"100\"/ value=\"\"></td>";
						}else{
							appendHtml = appendHtml+"<td>"+productNames[i]+"<input type='hidden' value='"+eids[i]+"' name='geAddServiceActivity.geActivitiesProducts["+i+"].eid'/></td>";
						}
