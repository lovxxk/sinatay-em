				...
				<c:forEach items="${kinds}" var="kinds" begin="0" step="1" varStatus="status">
					<tr id="tr_${kinds.id.kindCode}" class="${status.index%2 == 0?'':'search_tr_ou'}">
						<td class="td_border_body_top_none" nowrap>
							<input type="checkbox" name="checkChild" checked="checked" id="${kinds.id.kindCode}" onclick="checkSingleRow();" value="${kinds.id.riskCode}@${geKind.id.kindCode}@${geKind.kindCName}">
						</td>
						<td class="td_border_body_left_top_none" nowrap style='height: 30px'>${geKind.id.riskCode}</td>
						<td class="td_border_body_left_top_none" nowrap>${kinds.id.kindCode}&nbsp;</td>
						<td class="td_border_body_left_top_none" width="200px" nowrap>${geKind.kindCName}</td>
					</tr>
				</c:forEach>
