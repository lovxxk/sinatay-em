...
tt.moveToPos = function(srcE, targetE) {
	...
	srcE.style.top = targetpostion.t - 3;// - srcE.currentStyle.borderTopWidth - srcE.style.marginTop;
	srcE.style.left = targetpostion.r;// - srcE.currentStyle.borderLeftWidth - srcE.style.marginLeft;
...
//			tt.addCls(this.e, tt.Conf.errInputCls);
			...
			//	this.e.parentNode.insertBefore(span, this.e);
				this.e.parentNode.appendChild(span, this.e);
...
tt.BV = tt.C.ext({
	...
	/**
	 * 移除字段 用法:xx.rm("name1", "name2", "name3"...);或xx.rmName("name1", "name2", "name3"...);
	 */
	rmName : function() {
		return this._rm("name", arguments);
	},
	/**
	 * 移除字段 用法:xx.rm(fields1, fields2, ...);
	 */
	rmField:function(){
		return this._rm(null,arguments);
	},
	/**
	...
	attachE:function(e){
		...
		tt.Util.addEventHandler(e, "resize", tt.vf.resizeWindow);
		
		for (var x in tt.Conf.validateOn) {
			if (tt.Conf.validateOn[x] != 'focus' && tt.Conf.validateOn[x] != 'keyup'){
				tt.Util.addEventHandler(e, tt.Conf.validateOn[x],
					new _hr(e).h);
			} else if (e.tagName && e.tagName.toUpperCase() != 'SELECT') {
				tt.Util.addEventHandler(e, tt.Conf.validateOn[x],
					new _hr(e).h);
			}
		}
...
tt.CV = tt.BV.ext({
	...
	set : function(cmpType, oper, fOrV, showCmpVal) {
		...
		if (typeof fOrV == "string" || typeof fOrV == 'number') {
			this.cmpV = fOrV;
