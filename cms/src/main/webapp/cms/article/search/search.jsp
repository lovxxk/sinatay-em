...
String docIDs =(String)request.getAttribute("docIDs") == null ? "null" : (String)request.getAttribute("docIDs");
...
if("".equals(docIDs)){
	response.sendRedirect(path+"/cms/global/ui/info.jsp?message=noRecord");
}
...
<title>search.jsp</title>
	...
	function doUpdate(ID,docType){
		if(docType=="0"){
			window.open("${ctx}/articleManage/frmUpdate.do?ID="+ID,"_blank","top=100, left=100, width=950,height=800,toolbar=no,scrollbars");
		}else if(docType=="1"){
			window.open("${ctx}/articleManage/frmUpdateLink.do?ID="+ID,"_blank","top=100, left=100, width=950,height=375,toolbar=no,scrollbars");
		}
	}
	</script>
</head>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="table_Show">
	...
	<tr id="tr<%=i+1%>">
		...
		<td class="td_body"><a onclick="doUpdate('<%=document.getDocID()%>','<%=docType%>')" style="cursor:hand;text-decoration:underline;"><font color=blue><%=document.getDocTitle()%>&nbsp;</font></a>&nbsp;</td>
