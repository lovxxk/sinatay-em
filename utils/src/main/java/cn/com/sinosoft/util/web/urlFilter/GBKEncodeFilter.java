...
public class GBKEncodeFilter extends OncePerRequestFilter{

	 private static final Logger log = LoggerFactory.getLogger(GBKEncodeFilter.class);
		...
		protected void doFilterInternal(
			...
			if(url.contains("bgurl")||url.contains("pgurl")){//上银连返回的地址要使用UTF-8
				request.setCharacterEncoding("utf-8");
			}
			if (request.getServletPath().startsWith("/register/")
					|| request.getServletPath().startsWith("/login/")
					|| request.getServletPath().startsWith("/edit/")
					|| request.getServletPath().startsWith("/edit/")) {
//				request.setCharacterEncoding("UTF-8");
//				response.setCharacterEncoding("UTF-8");
			}
