...
/**
 * FastMd5InputStream, a subclass of FilterInputStream implementing FastMd5
...
public class FastMd5InputStream extends FilterInputStream {
	/**
	 * FastMd5 context
	 */
	private FastMd5 md5;
	...
	/**
	 * Creates a FastMd5InputStream
	 * 
	 * @param in
	 *            The input stream
	 */
	public FastMd5InputStream(InputStream in) {
		super(in);
	...
	public int read() throws IOException {
		...
		if ((c & ~0xff) != 0) {
			System.out.println("FastMd5InputStream.read() got character with (c & ~0xff) != 0)!");
		} else {
			md5.Update(c);
		}
	...
	public static void main(String[] arg) {
		try {
			...
			// //////////////////////////////////////////////////////////////
			//
			// usage: java com.twmacinta.util.FastMd5InputStream
			// [--use-default-md5] [--no-native-lib] filename
			//
			// ///////

			// determine the filename to use and the FastMd5 impelementation to
			// use
			...
			// Use the default FastMd5 implementation that comes with Java

			if (use_default_md5) {
				InputStream in = new BufferedInputStream(new FileInputStream(filename));
				java.security.MessageDigest digest = java.security.MessageDigest.getInstance("FastMd5");
				...
				// Use the optimized FastMd5 implementation
				...
				FastMd5InputStream in = new FastMd5InputStream(new BufferedInputStream(new FileInputStream(filename)));
				while ((num_read = in.read(buf)) != -1)
					;
